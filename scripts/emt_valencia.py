...
def fetchStops(lines) -> list[c.StopObject]:
    ...
    for line in lines:
        print("Fetching stops for line " + str(line.id))
        routesURL = (
            API_URL
            + "sentidos_ruta_linea.php"
            + "?usuario=7gH8m45w7A"
            + "&linea="
            + str(line.id)
            + "&lang=es"
        ...
        try:
            ...
            try:
                ...
                logging.warning("Error found fetching routes for line: " + str(line.id))
                print("Input:")
        ...
        for route in routes:
            r = requests.get(
                API_URL + "paradas_linea.php" + "?usuario=7gH8m45w7A&"
                "&linea="
                + str(line.id)
                + "&ruta="
                + route["ruta"]
            ...
            try:
                ...
                logging.warning("Error found fetching stops on line: " + str(line.id))
                print("Input:")
                print(r.content.decode("utf-8"))
                print("URL: " + routesURL)
                continue
            ...
            for stop in stops:
                ...
                stopLineEmblems = [match.value for match in lineQuery.find(stop)]
...
def fetchLinePath(line: c.LineObject):
    print("Fetching path for line " + line.emblem)

    r = requests.get(
        "https://www.emtvalencia.es/ciudadano/servicios/ruta_linea.php?usuario=7gH8m45w7A&linea="
        + str(line.id)
    )
    query = jsonpath_ng.parse("$.features[*].geometry.coordinates[*]")

    segments: list[list[list[float]]] = [match.value for match in query.find(r.json())]

    geoJson = c.makeGeoJson(segments, True)
    line.path = geoJson
